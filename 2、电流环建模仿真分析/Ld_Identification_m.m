...
%仿真辨识
out = sim("Ld_Identification_sim.slx");
output = out.Id.Data(Step + 1:end) - mean(out.Id.Data(Step + 1:end));
[sys_tf_0,Gest_0] = DataProcess(input_prbs - DC,output,k,m,Ts);
Ld_0 = -Rs*Ts./log((pole(sys_tf_0)));
fprintf('辨识结果：%.5f mH\n相对偏差：%.5f%%\n\n', ...
    Ld_0(1)*1000, (Ld_0(1)-Ld)/Ld*100);
...
%     out = sim("Ld_Identification_sim.slx");
%     output = out.Id.Data(Step + 1:end) - mean(out.Id.Data(Step + 1:end));
%     [sys_tf,~,r_AF,r_PF] = DataProcess(input_prbs - DC,output,k,m,Ts);
%     close all;
